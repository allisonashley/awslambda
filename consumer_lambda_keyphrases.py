...
LOG.info(f"STARTED Imported Lambda")
...
def names_to_wikipedia(names):
    ...
    wikipedia_snippit = []
    for name in names:
        wikipedia_snippit.append(wikipedia.summary(name, sentences=5))
    df = pd.DataFrame(
        {
            'names':names,
...
def detect_key_phrases(row):
    ...
    LOG.info(f"FOUND Key Phrases: {payload}")
    key_phrase = payload['KeyPhrases']
    return key_phrase

def apply_key_phrases(df, column="wikipedia_snippit"):
    """Uses Pandas Apply to Detect Key Phrases"""
    LOG.info(f"INSIDE Key Phrases")
    df['KeyPhrases'] = df[column].apply(detect_key_phrases)
...
def write_s3(df, bucket, name):
    ...
    LOG.info(f"INSIDE S3")
    csv_buffer = StringIO()
    df.to_csv(csv_buffer)
    s3_resource = boto3.resource('s3')
    filename = f"{name}_key_phrase.csv"
...
def lambda_handler(event, context):
    ...
    LOG.info(f"Key Phrases from FANG companies: {df.to_dict()}")

    # Write result to S3
    write_s3(df=df, bucket="fangkeyphrases1", name=names) 
